...
async def read_receivable(receivable_id: UUID, db: AsyncSession = Depends(get_db)):
    service = ReceivableService(db)
...
async def delete_receivable(receivable_id: UUID, db: AsyncSession = Depends(get_db)):
    service = ReceivableService(db)
